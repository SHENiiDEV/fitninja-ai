...
export default function Billing({ tokens, ratePerToken = 1.00, presetPackages, bankDetails, payments = [], status }) {
        ...
        >
            ...
            <div className="py-8">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    {/* B2B Transaction History & Official PDF Invoices */}
                    <div className="mt-12 rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-xl backdrop-blur-xl">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
                            <div>
                                <h3 className="text-xl font-bold text-white">3. Official B2B Tax Invoices & Transaction History</h3>
                                <p className="text-xs text-slate-400">Download official UK VAT / Reverse Charge receipts (CHANGE IT UP SERVICES LTD)</p>
                            </div>
                        ...
                        {payments && payments.length > 0 ? (
                                    ...
                                    <tbody className="divide-y divide-slate-800/60 font-medium">
                                        {payments.map((tx) => (
                                            <tr key={tx.id} className="hover:bg-slate-950/40 transition-colors">
                                                <td className="py-4 px-3 text-slate-300">{tx.created_at}</td>
                                                <td className="py-4 px-3 font-mono font-bold text-white">{tx.gateway_reference}</td>
                                                <td className="py-4 px-3 text-slate-200">{tx.service_name}</td>
                                                <td className="py-4 px-3 font-bold text-emerald-400">{formatPrice(tx.amount)}</td>
                                                ...
                                                <td className="py-4 px-3 text-right">
                                                    <a
                                                        href={route('billing.invoice', tx.id)}
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        className="cursor-pointer inline-flex items-center gap-1.5 rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-bold text-slate-200 hover:border-emerald-500 hover:text-emerald-400 transition-all shadow-sm"
                                                    >
                            ...
                            <div className="py-8 text-center text-xs text-slate-500">
                                No transaction records found yet. Purchase an AI token package above to generate your first official B2B PDF invoice.
                            </div>
                        )}
                    </div>
                </div>
